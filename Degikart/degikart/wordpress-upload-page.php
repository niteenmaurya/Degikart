<?php
/*
Template Name: WordPress Upload Page
*/
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $regular_price = sanitize_text_field($_POST['regular-price']);
    $extended_price = sanitize_text_field($_POST['extended-price']);
    $support_price = sanitize_text_field($_POST['support-price']);
    ...
    $demo_url = esc_url_raw($_POST['demo-url']); // Use esc_url_raw for URLs
    $tags = sanitize_text_field($_POST['tags']);
    $message_to_reviewer = sanitize_textarea_field($_POST['message-to-reviewer']); // Use sanitize_textarea_field for textareas
    $version = sanitize_text_field($_POST['version']);
    $plugin_file = $_FILES['plugin-file']; // File uploads should be handled differently
    $plugin_name = sanitize_text_field($_POST['plugin-name']);
    $requires_premium_plugins = sanitize_text_field($_POST['requires-premium-plugins']);
...
function generate_unique_filename($file) {
    ...
    return $unique_name_with_original;
            ...
            // Handle the thumbnail and theme preview using default WordPress upload
            ...
            $plugin_file_upload = wp_handle_upload($plugin_file, array('test_form' => false));
            ...
            if ($main_files_url && !is_wp_error($main_files_url) && $thumbnail_upload && !isset($thumbnail_upload['error']) && $theme_preview_upload && !isset($theme_preview_upload['error'])) {
                ...
                $plugin_file_url = $plugin_file_upload['url'];
                ...
                if ($post_id) {
                    ...
                    update_post_meta($post_id, 'requires_premium_plugins', $requires_premium_plugins);
                    update_post_meta($post_id, 'plugin_file_url', esc_url_raw($plugin_file_url));
                    ...
                    // Handle the theme preview as a file in the media library
...
<div class="form-group">
    <div class="label-container">
        ...
        <input type="text" id="theme-name" name="theme-name" class="input-text" maxlength="50" required>
        <p class="guide-instructions">
            Title should be under 100 characters. No HTML or emojis allowed.
        </p>
    </div>
</div>
...
<div class="label-container">
    ...
    <div class="form-group">
    <div class="label-container">
        <label for="thumbnail"><?php _e('Thumbnail (JPEG/PNG, 80x80px):', 'degikart'); ?></label>
        </div>   <div class="input-container">
    ...
    <div class="form-group">
    <div class="label-container">
        <label for="theme-preview"><?php _e('Theme Preview (3:2 Ratio, Recommended 2340x1560px):', 'degikart'); ?></label>
        </div>   <div class="input-container">
    ...
    <div class="label-container">
        <label for="regular-price"><?php _e('Regular License Price:', 'degikart'); ?></label>
              ...
              <input type="number" id="regular-price" name="regular-price" class="input-number" required>
              </div> </div>
    ...
    <div class="label-container">
        ...
        <div class="input-container">
        <input type="number" id="extended-price" name="extended-price" class="input-number" required>
    ...
    <div class="form-group">
    <div class="label-container">
        <label for="support-price"><?php _e('12 Months Support Price:', 'degikart'); ?></label>
        </div>  <div class="input-container">
             <input type="number" id="support-price" name="support-price" class="input-number" required>
             </div> </div>
    ...
    <div class="label-container">
        <label for="last-update"><?php _e('Last Update:', 'degikart'); ?></label>
              ...
              <input type="date" id="last-update" name="last-update" class="input-date" required>
              </div></div>
    ...
    <div class="label-container">
        <label for="published"><?php _e('Published:', 'degikart'); ?></label>
             ...
             <input type="date" id="published" name="published" class="input-date" required>

             </div> </div>

    <div class="form-group">
    <div class="label-container">
        <label><?php _e('High Resolution:', 'degikart'); ?></label>
        </div>  <div class="input-container">
        <input type="checkbox" id="high-resolution" name="high-resolution" value="1">
        
        <label for="high-resolution"><?php _e('Yes', 'degikart'); ?></label>
        </div></div>
    ...
    <div class="label-container">
        <label for="demo-url"><?php _e('Demo URL:', 'degikart'); ?></label>
             ...
             <input type="url" id="demo-url" name="demo-url" class="input-text">
             </div> </div>
    ...
    <div class="label-container">
        <label for="tags"><?php _e('Tags:', 'degikart'); ?></label>
             ...
             <p class="guide-instructions"  >
    You can enter up to 15 keywords that best represent your item. Focus on its key features, uses, and design. 
    Make sure to use lowercase letters and separate each keyword with a comma (e.g., photography, gallery, modern, jquery, wordpress theme).
</p>
    ...
    <div class="form-group">
    <div class="label-container">
        <label for="plugin-file"><?php _e('Plugin File (if applicable):', 'degikart'); ?></label>
        </div> <div class="input-container">
            <input type="file" id="plugin-file" name="plugin-file" class="input-file">
            </div></div>

    <div class="form-group">
    <div class="label-container">
        <label for="plugin-name"><?php _e('Plugin Name (if applicable):', 'degikart'); ?></label>
        </div>
        <div class="input-container">
            <input type="text" id="plugin-name" name="plugin-name" class="input-text" placeholder="<?php _e('Enter plugin name', 'degikart'); ?>">
            </div>
        </div>

    <div class="form-group">
    <div class="label-container">
        <label for="requires-premium-plugins"><?php _e('Requires Premium Plugins:', 'degikart'); ?></label>
        </div>
        <div class="input-container">
             <input type="text" id="requires-premium-plugins" name="requires-premium-plugins" class="input-text" placeholder="e.g. Elementor Pro">
             </div>
    </div>
    ...
    <div class="form-group">
        ...
        <?php
            // Fetch categories from the custom taxonomy 'wordpress_category'
            wp_dropdown_categories(array(
                'taxonomy'         => 'wordpress_category', // Specify the custom taxonomy
                'name'             => 'category',            // Name attribute for the dropdown
                'id'               => 'category',            // ID for the dropdown
                'selected'         => '',                    // Default selected value
                'show_option_none' => __('Select Category', 'degikart'), // Placeholder option
...
<script type="text/javascript">
    document.querySelector('.form-main').addEventListener('submit', function(event) {
        ...
        // Optionally, prevent the default form submission to ensure this action runs first
