...
// Define the post types you want to display
$post_types = ['wordpress', 'blogger', 'plugin', 'ecommerce']; // Array of custom post types

// Get the current page number for pagination
$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;

// Get sorting parameters from the URL (defaults to price and ascending)
$orderby = isset($_GET['orderby']) ? $_GET['orderby'] : 'price';
$order = isset($_GET['order']) ? $_GET['order'] : 'asc';
$meta_key = ($orderby == 'price') ? 'regular_price' : 'sales_count';

// Modify the query for search results to include all post types
$args = array(
    'post_type' => $post_types, // Use the defined array of custom post types
    'posts_per_page' => 12,     // Set the number of posts per page
    'paged' => $paged,          // Use current page for pagination
    'meta_key' => $meta_key,    // Sorting by price or sales count
    'orderby' => 'meta_value_num', // Ensure sorting by numeric meta values
    'order' => $order,          // Sorting order: 'asc' or 'desc'
    's' => get_search_query(),  // Make sure search results are filtered based on the search query
);

// Execute the custom query for the search results
$custom_query = new WP_Query($args);

// Get the total count of matching posts (based on the same query parameters)
    ...
    <main class="site-content-area" id="content">
        <div class="sorting-buttons">
            <div class="total-products-count">
                <p><?php echo sprintf(__('Items: %d', 'degikart'), $total_count); ?></p>
            </div>
        ...
        <div class="site-cont" id="cont-site">
            ...
            if ($custom_query->have_posts()) :
                while ($custom_query->have_posts()) : $custom_query->the_post();
                    if (in_array(get_post_type(), $post_types)) :  // Check if the post type is one of the allowed types
