...
$reviews_count = count(get_comments(array('post_id' => get_the_ID())));
...
if (have_posts()) :
    ...
    $compatible_browsers = isset($meta['compatible_browsers'][0]) ? esc_html($meta['compatible_browsers'][0]) : '';
    $compatible_with = isset($meta['compatible_with'][0]) ? esc_html($meta['compatible_with'][0]) : '';
    $themeforest_files_included = isset($meta['themeforest_files_included'][0]) ? esc_html($meta['themeforest_files_included'][0]) : 'N/A'; // Added this
    $columns = isset($meta['columns'][0]) ? esc_html($meta['columns'][0]) : '';
    $layout = isset($meta['layout'][0]) ? esc_html($meta['layout'][0]) : '';
    ...
    $theme_preview_url = isset($meta['theme_preview_url'][0]) ? esc_url($meta['theme_preview_url'][0]) : '';
    $views = get_post_meta(get_the_ID(), 'product_views_count', true);
     
    $requires_premium_plugins = isset($meta['requires_premium_plugins'][0]) ? esc_html($meta['requires_premium_plugins'][0]) : '';
    $message_to_reviewer = isset($meta['message_to_reviewer'][0]) ? esc_html($meta['message_to_reviewer'][0]) : '';
    $high_resolution = isset($meta['high_resolution'][0]) ? esc_html($meta['high_resolution'][0]) : 'No';

    $plugin_name = isset($meta['plugin_name'][0]) ? esc_html($meta['plugin_name'][0]) : '';

    $plugin_file = isset($meta['plugin_file'][0]) ? esc_html($meta['plugin_file'][0]) : 'N/A';
...
<div class="sales-bar" id="item-rate">
                    ...
                    <div class="post-body" itemprop="text">
                            <?php if ($thumbnail_url) : ?>
                                <div class="post-image">
                                    <img src="<?php echo esc_url($thumbnail_url); ?>" alt="<?php esc_attr_e('Thumbnail', 'degikart'); ?>" />
                                </div>
                            <?php endif; ?>
                            <div class="post-preshot">
                                <p><a href="<?php echo $demo_url; ?>"><?php _e('Live Preview', 'degikart'); ?></a></p>
                                <p><a href="<?php echo $theme_preview_url; ?>"><?php _e('View Screenshot', 'degikart'); ?></a></p>
                            </div>
            ...
            <div class="product-right-sidebar">
                <div id="product-sidebar">
                    <form id="product-form">
                        <div>
                            <select id="license" name="license" onchange="updateTotalPrice()">
                                <option value="regular"><?php printf(__('Regular License - $%s', 'degikart'), $regular_price); ?></option>
                                <option value="extended"><?php printf(__('Extended License - $%s', 'degikart'), $extended_price); ?></option>
                            </select>
                            <span id="total-price" style="float: right; font-size: 18px; font-weight: 600;">$0</span>
                        </div>
                        <div class="monprice">
                            <label>
                                <input type="checkbox" id="support" onclick="updateTotalPrice()"> <?php _e('12 Months Support', 'degikart'); ?>
                            </label>
                            <span id="support-price" style="float: right;">$<?php echo $support_price_12_months; ?></span>
                        </div>
                        <div class="buttontotal">
                        <button type="button" onclick="addToCart(<?php echo get_the_ID(); ?>)">Add To Cart</button>
                        </div>
                    </form>
                </div>
    ...
    <table class="table-style">
        <?php if (!empty($version)): ?>
            <tr>
                <th><?php _e('Version:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($version); ?></td>
            </tr>
        <?php endif; ?>
        ...
        <?php if (!empty($compatible_browsers)): ?>
            <tr>
                <th><?php _e('Compatible Browsers:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($compatible_browsers); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($compatible_with)): ?>
            <tr>
                <th><?php _e('Compatible With:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($compatible_with); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($themeforest_files_included)): ?>
            <tr>
                <th><?php _e('Files Included:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($themeforest_files_included); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($columns)): ?>
            <tr>
                <th><?php _e('Columns:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($columns); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($layout)): ?>
            <tr>
                <th><?php _e('Layout:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($layout); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($plugin_name)): ?>
            <tr>
                <th><?php _e('Plugin Name:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($plugin_name); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($requires_premium_plugins)): ?>
            <tr>
                <th><?php _e('Requires Premium Plugins:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($requires_premium_plugins); ?></td>
            </tr>
        <?php endif; ?>
        ...
        <?php if (!empty($category)): ?>
            <tr>
                ...
                <td class="rtl"><?php echo esc_html($category); ?></td>
            </tr>
        <?php endif; ?>

        <?php if (!empty($message_to_reviewer)): ?>
            <tr>
                <th><?php _e('Message to Reviewer:', 'degikart'); ?></th>
                <td class="rtl"><?php echo esc_html($message_to_reviewer); ?></td>
            </tr>
        <?php endif; ?>
    </table>
</div>
        ...
        <script type="application/ld+json">
{
    ...
    "aggregateRating": {
        ...
        "reviewCount": "<?php echo esc_html($reviews_count); ?>",
        "ratingDistribution": {
            "1": "<?php echo esc_html($one_star_count); ?>",
            "2": "<?php echo esc_html($two_star_count); ?>",
            "3": "<?php echo esc_html($three_star_count); ?>",
            "4": "<?php echo esc_html($four_star_count); ?>",
            "5": "<?php echo esc_html($five_star_count); ?>"
        }
    }
}
</script>
